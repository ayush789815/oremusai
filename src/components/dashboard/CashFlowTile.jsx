...
export default function CashFlowTile({ data = [] }) {
  ...
  const netFmt  = net === 0 ? `${sym}0` : `${net >= 0 ? '+' : ''}${sym}${Math.abs(net)}k`;
  ...
  return (
    <Tile padding="p-4" className="h-full">
      <div className="flex items-center justify-between mb-2">
        ...
        {data.length > 0 && <Badge tone={netTone} dot>{netFmt} net</Badge>}
      </div>
      <div className="h-[140px] -mx-1">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 5, right: 5, left: -15, bottom: 0 }}>
            ...
            <YAxis tick={AxisStyle} axisLine={false} tickLine={false} tickFormatter={(v) => `${sym}${v}k`} />
            <Tooltip
              contentStyle={{
                borderRadius: 8,
